...
struct SnapshotStructure<SectionIdentifierType, ItemIdentifierType> where SectionIdentifierType: Differentiable, SectionIdentifierType: Hashable, ItemIdentifierType: Differentiable, ItemIdentifierType: Hashable {
    struct Item: Differentiable, Equatable {
        var item: ItemIdentifierType
        var isReloaded: Bool
        
        var differenceIdentifier: ItemIdentifierType.DifferenceIdentifier {
        ...
        func isContentEqual(to source: Item) -> Bool {
            return !isReloaded && item.isContentEqual(to: source.item)
        }
    }
    
    struct Section: DifferentiableSection, Equatable {
        var section: SectionIdentifierType
        var elements: [Item] = []
        var isReloaded: Bool
        
        var differenceIdentifier: SectionIdentifierType.DifferenceIdentifier {
        ...
        init<C: Swift.Collection>(source: Section, elements: C) where C.Element == Item {
            self.init(id: source.section, items: Array(elements), isReloaded: source.isReloaded)
        }
        
        func isContentEqual(to source: Section) -> Bool {
            return !isReloaded && section.isContentEqual(to: source.section)
        }
    }
    ...
    func items(in sectionID: SectionIdentifierType, file: StaticString = #file, line: UInt = #line) -> [ItemIdentifierType] {
        ...
        return sections[sectionIndex].elements.map { $0.item }
    }
    
    func section(containing itemID: ItemIdentifierType) -> SectionIdentifierType? {
        return itemPositionMap()[itemID]?.section.section
    }
...
private extension SnapshotStructure {
    ...
    func sectionIndex(of sectionID: SectionIdentifierType) -> Array<Section>.Index? {
        return sections.firstIndex { $0.section.isEqualHash(to: sectionID) }
    }
